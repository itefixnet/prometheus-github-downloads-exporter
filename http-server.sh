#!/bin/bash
#
# GitHub Downloads Prometheus Exporter HTTP Server
...
LISTEN_PORT="${LISTEN_PORT:-9168}"
LISTEN_ADDRESS="${LISTEN_ADDRESS:-0.0.0.0}"
EXPORTER_SCRIPT="${EXPORTER_SCRIPT:-${SCRIPT_DIR}/github-downloads-exporter.sh}"
...
start_server() {
    log "Starting GitHub Downloads Prometheus Exporter HTTP Server"
    ...
    local handler_script="/tmp/github_downloads_handler_$$"
...
case "$path" in
    ...
    "/health")
        ...
        ;;
    "/")
        html="<!DOCTYPE html><html><head><title>GitHub Downloads Exporter</title></head><body><h1>GitHub Downloads Prometheus Exporter</h1><p><a href=\"/metrics\">Metrics</a> | <a href=\"/health\">Health</a></p><p>Monitoring GitHub repository download statistics.</p></body></html>"
...
stop_server() {
    log "Stopping GitHub Downloads Prometheus Exporter HTTP Server"
...
case "${1:-start}" in
    ...
    "test")
        ...
        ;;
    "version")
        echo "GitHub Downloads Exporter HTTP Server v1.0.0"
        ;;
    "help"|"-h"|"--help")
        ...
        echo "  LISTEN_PORT       - HTTP server port (default: 9168)"
        echo "  LISTEN_ADDRESS    - HTTP server bind address (default: 0.0.0.0)"
        echo "  EXPORTER_SCRIPT   - Path to exporter script (default: ./github-downloads-exporter.sh)"
